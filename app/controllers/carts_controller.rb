class CartsController < ApplicationController
  ...
  def update_session_cart
    ...
    if @product.is_out_of_stock? params[:quantity].to_i
      flash.now[:danger] = t "cart.product_out_of_stock"
    else
      @current_cart = current_cart
      if @product.update_attributes(product_quantity: @product.product_quantity - params[:quantity].to_i)
        @current_cart.add @product, params[:quantity]
      else
        flash[:danger] = t "cart.add_error"
      end
    end
  end
  ...
  def destroy_session_cart
    load_product
    @current_cart = current_cart
    @product.update_attribute :product_quantity,
      @product.product_quantity + @current_cart.quantity(@product)
